...
const END_DATE_OFFSET = 14400000; // 4 hours in milliseconds (main prize distribution)
const VESTING_INTERVAL = 3600000; // 1 hour in milliseconds (small prize distribution)
...
export function constructor(): void {
  ...
  // Schedule automated execution for NFT transfer every 30 minutes
  Timer.createRecurring("scheduleDailyExecution", 1800000); // Trigger every 30 minutes
}
...
// Automated Function to execute NFT transfers every 30 minutes
export function scheduleDailyExecution(): void {
  ...
  if (now < bytesToU64(Storage.get(LAST_TICKET_SOLD_TIMESTAMP_KEY)).toU32() + END_DATE_OFFSET && (lastExecution == 0 || now - lastExecution >= 1800000)) {
    transferNFTs();
...
// Function to distribute main prizes in vesting mode every 4 hours
export function distributeMainPrizes(): void {
  ...
  // Vesting over a defined number of intervals (e.g., 10 intervals)
